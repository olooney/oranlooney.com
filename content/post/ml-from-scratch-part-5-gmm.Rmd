...
structure. In particular, there is at least one cluster of data clearly
separated from the rest. Surely there is some algorithm which can discover
this structure! On the other hand, none of the techniques we have studied 
in this series so far are even able to make a start: they all assume that
we have a clear definition of the thing that we are trying to predict, and
further more they assume that we know it for every example in the training set.
A problem of the form, "find **any** interesting relationships or structure"
is simply too vague. 

That is because all of the algorithms we've looked at so far have been "supervised"
...
called the "unsupervised" learning problem, and call for a different set of techniques
...
including non-linear techniques like [t-SNE][TSNE] and most examples of representation
...
which maps onto exactly one of $k$ possible classes. The classic example of a clustering algorithm
is [$k$-means][KM]. Reducing rich, multivariate data to a small finite number of possibilities
seems extreme, but for that same reason it can be extremely clarifying as well. The specific
algorithm we will implement in this article will be a clustering algorithm, and we will return
to dimensional reduction at a later date.
...
Very broad and powerful algorithm. Also used for Factor Analysis and the [Fellegi-Sunter][FS] record linkage algorithm,
...
One good resource on GMM and the EM algorithm I used was this [Stanford Lecture by Andrew Ng](https://youtu.be/ZZGTuAkF-Hw?t=2108). I've linked to the part of the lecture where he shows this update step in particular but the whole lecture is worth watching. Another
is this deck: http://www.cs.cmu.edu/~guestrin/Class/10701-S07/Slides/em-baumwelch.pdf the highlight of which is a very simple
example you can work by hand; this is a great way to build intuition. 
