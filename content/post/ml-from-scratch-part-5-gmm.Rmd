...
Consider an dataset like the following:

![Unlabled Data](/post/ml-from-scratch-part-5-gmm_files/unlabled.png)

On the one hand, it is visually apparent that these data have a certain
structure. In particular, there is at least one cluster of data in the lower right which is clearly
separate from the rest. Unfortunately, it turns out that none of the techniques we have studied 
in this series so far are even able to make a start: they all assume that
we have a "labeled" traing set, which requires that we not only have clear definition of the thing that we are trying to predict, we
must in fact already know the correct answer for every example in the training set.
...
learning problems, where the training set consists of *labeled* pairs $(\mathbf{X}, \mathbf{Y})$
and the tast was to predict $\mathbf{Y}$ from $\mathbf{X}$. The problem of discovering
interesting structure or relationships from *unlabeled* examples $\mathbf{X}$ is
...
means that the several different gaussian distributions, all with different mean vectors $mu_i$ and different
covariance matrices $\Sigma_i$, are combined by taking the weighted sum of the probability density functions:

\[ \begin{align}
    f_{GMM}(\mathbf{x}) = \sum^k_{i=1} \phi f_{\mathcal{N}(\mathbf{\mu}_i, \Sigma_i)}(\mathbf{x}) \tag{1}
   \end{align}
\]
...
We can make this fully explicit by expanding the definition of the p.d.f. of the multivariate normal distribution:

\[ \begin{align}
    C_i & \sim \mathcal{N}(\mathbf{\mu}_i, \Sigma_i) \tag{3} \\
    f_{C_i}(\mathbf{x}) &= \frac{1}{\sqrt{ (2\pi)^k |\Sigma_i| }} e^{(\mathbf{x} - \mathbf{\mu}_i)^T \Sigma_i^{-1} (\mathbf{x} - \mathbf{\mu}_i)} \tag{4} \\
    P(\mathbf{x}) & = \sum^k_{i=1} \phi_i f_{C_i}(\mathbf{x}) \tag{5}
   \end{align}
\]

It is obvious that a model of this form could represent a distribution with multiple distinct clusters, but it
is less clear how, given a dataset $\mathbf{X}$, we can choose parameters 

\[\Theta = \{ \mathbf{\mu}_1, \Sigma_1, ..., \mathbf{\mu}_k, \Sigma_k \} \tag{6} \]

The first step is decide that we will use the maximum likelihood estimate for the parameters. Because the $n \times m$ example matrix $\mathbf{X}$
is assumed to be a realization of $n$ i.i.d. samples from $f_{GMM}(\mathbf{x})$, we can write down our likelihood function as

\[
  \mathcal{L}(\Theta; \mathbf{X}) = \prod_{j=1}^n \sum^k_{i=1} \phi_i f_{C_i}(\mathbf{X}_j) \tag{7}
\]

It is at this point we encounter an empass. In the [past][MLFS1], we've been able to simplify such likelihood functions by 
first taking a log to turn the product into a sum and also get rid of the exponent, reducing the problem to ordinary least squares.
Here, however, because of the inner sum over classes, there is no obvious way to simplify this to a tractable form where
the gradient might be calculated analytically. We need a new approach.
