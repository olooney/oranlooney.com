...
Obtaining a formula for $P(\mathbf{X}_i|C_i=j)$ requires a little more work. We know that the unconditional probability is given by the parameter $\boldsymbol{\phi}$:
...
The first thing we need to do is introduce a parameter to model the unknown latent variables $C_i$. Because $C_i$ can take
...
The EM algorithm then proceeds by iterations, with each iteration being divided into two steps, the E-step and the M-step.
I will describe these in broad strokes first, so you can get a feel for the overall intent of the algorithm, then we will
examine each in later sections.
...
for the center and shape of each cluster. Concretely, we use $w_{ij}$ as *weights* and then update $\boldsymbol{\mu}_j$ and $\Sigma_j$ by taking weighted averages over $X$. For example, if $w_11 = 0.01$ and $w_11 = 0.99$ we know that the data point $X_1$ 
...
likelihood can only increase and never decrease. This can be viewed as an application of [coordinate ascent][CD].
These maxima are not unique, and GMM will tend to converge to different final solutions depending on initial conditions. 

One  resource on GMM and the EM algorithm I used was this [Stanford lecture by Andrew Ng](https://youtu.be/ZZGTuAkF-Hw?t=2108). I've linked to the part of the lecture where he shows this update step because that is most relevant to implementing the algorithm but the whole lecture is worth watching if you want to understand the concepts. Another good resource is this [slide deck][SD]; I found that
...
index $i_j$ uniformly from $1$ to $n$ for each class and then initialize $boldsymbol{\mu}_j = X_i_j$. 
...
We'll use the famous [iris][ID] dataset as a test case. This actually is the same dataset used as a motivating example at the begining
of the article, though I did not name it at that time. The iris dataset has labels, but we won't expose them to the GMM model.
However, we will use these labels later to discuss the question, "were we able to discover the class labels through unsupervised learning?"
...
good job placing the visually separate points in the lower right into its own cluster, but the story with the
