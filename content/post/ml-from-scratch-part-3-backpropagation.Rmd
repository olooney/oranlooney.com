...
The 3Blue1Brown video is notable for one pedagogical trick in particular:
by considering a "one-dimension" neural network with only a single node
in each layer, he was able to present the key idea of using the chain
rule using only high school calculus.
...
The forward pass of an simple $L$-layer neural net is given recursively as:
...
connecting layers, and $b^{(i)}$ are bias vectors.  $X$ is a matrix of data
with one row per observation and one column per feature; while the final
...
For the backwards pass, therefore, we can use the chain rule. For example,
let's do $W^{(1)}$:
...
Given the forward pass equations given above, it turns out that each of these
partial derivatives is straight-forward to calculate. You can verify for
yourself that:
...
\[ \frac{\partial z^{(i)}}{\partial a^{(i-1)}} = W^{(i)} \]

\[ \frac{\partial z^{(i)}}{\partial W^{(i)}} = a^{(i-1)} \]
...
The justification for introducing the (technically extraneous) concept of $z$
is found in how absolutely obvious and clear it makes the separate steps of the
forward and backwards pass. If we had to take $\partial a^{(i)} / \partial
a^{(i-1)}$ directly without going through $z$ we would have to think about a
non-linear function of a matrix all at once; with $z$ we're able to view it as
the element-wise application of a non-linear function (which is Calculus 101)
and the partial derivative of a linear expression with respect to a matrix
(which is Matrix Calculus 101; see the [Cookbook][MC]). 
...
In the next installment of the [Machine Learning From Scratch series][MLFS], we
will change tact and look at a completely different approach to non-linear
...
viewed as examples of [adaptive basis functions][ABF], and how this
