...
[logistic regression][MLFS2] to build a classifier which is able to represent
non-linear relationships and interactions between features: the neural network.
In particular I want to focus on one central algorithm which allows us to apply
gradient descent to deep neural networks: the backpropagation algorithm.
Gradient descent can be very fast which makes them very attractive from a
...
There's a lot of mysticism around neural networks but a lot of the perceived
complexity vanishes once you look at the math. A typical fully-connected
feed-forward $L$-layer neural net is given recursively in just four simple
equations, the first and last of which merely define how we get data in and
predictions out of the network:
...
(A brief aside about notation. Here, a superscript inside of parentheses is a
layer index; the parentheses are meant to distinguish it from an exponent. This
...
$\sigma()$ in this case. Start with a signal $X$. To propagate the signal
...
application function. Keep doing that until you reach the output layer.
...
Since every element of every matrix is a separate parameter, neural networks
tend to have a *lot* of parameters. (This implies that neural nets have high
[VC dimension][VCD] and can easily overfit if not watched closely.)

To make this abstract recurrence relation concrete, we can collapse this
recurrence relation into a single equation for any given depth $L$. For
example, with zero hidden layers ($L=1$) a neural net reduces to the equation
...
where we rely on the slightly non-obvious fact that $\sigma'(x) = \sigma(x)
...
By now you be starting to see a pattern emerge: as we go back layer by layer,
...
factors, and capped off by the final red factor.
...
Good toy datasets for testing a neural network are a bit hard to come by.  You
need something that exhibits strong non-linearities and interactions to prove
the advanced algorithm is solving a problem that a simpler algorithm would
choke on; *and* you need quite a bit of data before any non-linear classifier
has enough data to start to pick up on those second-order (or higher)
regularities. I've tested the above NN on the classic [MNIST handwritten
digit][MHD] problem but it worked too well on the training set and too poorly
on the training set; that would be fine if I wanted to talk about diagnosing
and solving regularization in this article, but it's a bit of a problem since I
want to talk about how adding layers to neural networks affects them. In the
end, instead of shopping around for an organic dataset that met all of my requirements,
I simply synthesized one to spec using scikit-learn's handy generators.
...
on the test set. It has two hidden layers of 8 nodes and 3 nodes respectively.
