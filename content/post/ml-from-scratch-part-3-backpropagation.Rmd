...
A full description of the forward pass of an simple $L$-layer neural net is given
recursively as:
...
from an exponent. This slightly verbose convention is used so that ordinary subscripts can be
used to refer to the individual elements of $W$ and $b$, for example,
$W_{jk}^{(i)}$ is the element in the $j$-th row of the $k$-th column of the
weight matrix $W^{(i)}$ for the $i$-th layer.)
...
connection matrix $W^{(i)}$ as well as the bias vectors $b^{(i)}$: $\Theta = (W^{(1)} ... W^{(L)}, b^{(1)} ... b^{(L)})$. 
Since every element of every matrix is a separate parameter, neural networks
tend to have a *lot* of parameters.

Concretely, we can collapse this recurrence relation into a single equation for
any given depth $L$. With zero hidden layers ($L=1$) a neural net reduces to the
equation for logistic regression:

\[ \hat{y} =\sigma(W^{(1)} X + b^{(1)}) \]

If a zero-hidden-layer neural net is also trained with log-loss, both
the model's representation and fitted parameters will be exactly the same
...
networks don't solve the bias-variance tradeoff for us, and they certainly
...
In the next installment in the [Machine Learning From Scratch series][MLFS], we will
