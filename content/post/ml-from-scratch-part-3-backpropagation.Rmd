...
In today's installment of [Machine Learning From Scratch][MLFS], I want to
build on the [logistic regression][MLFS2] to build a classifier which is able
to automatically linear non-linear relationships and interactions between
features: the neural network. In particular I want to focus on a central
algorithm which makes the neural network fast and easy to train and therefore
very attractive from an applied point-of-view: the backpropagation algorithm.

I don't think the world needs another Neural Net 101 blog post, so I will send
you to [other][GORM] authors who [already have][DL2] first-rate
[presentations.][3B1B] My intention with this article is simply to derive and
implement that backpropagation algorithm from scratch, and to discuss how we
can mitigate some of the dangers introduced when we move beyond logistic to
more complex representations.

Representation
--------------

A simple feed-forward $L$-layer neural net is given recursively as:
...
Where $\sigma()$ is a sigmoid function, $W^{(i)}$ are matrices of weights
connecting layers, and $b^{(i)}$ are bias vectors.  $X$ is the given matrix of
data with one row per observation and one column per feature; while the final
activation $a^{(L)}$ *is* our prediction $\hat{y}$. (Here, the superscript in
parentheses is a layer index and the parentheses are meant to distinguish it
from an exponent. This slightly verbose convention is used so that ordinary
subscripts can be used to refer to the individual elements of $W$ and $b$, for
example, $W_{jk}^{(i)}$ is the element in the $j$-th row of the $k$-th column
of the weight matrix $W^{(i)}$ for the $i$-th layer.)
...
a number of columns equal to the number of nodes on the previous layer and a
number of rows equal to the number of nodes of the next layer. In particular
that means $W^{(1)}$ must have a number of columns equal to the number of features
in the dataset $X$, while $W^{(L)}$ has only a single output row.

The parameters of the model are the combination of *all* the weights of every
connection matrix $W^{(i)}$ as well as the bias vectors $b^{(i)}$: $\Theta =
(W^{(1)} ... W^{(L)}, b^{(1)} ... b^{(L)})$.  Since every element of every
matrix is a separate parameter, neural networks tend to have a *lot* of
parameters.

We can collapse this recurrence relation into a single equation for any given
depth $L$. For example, with zero hidden layers ($L=1$) a neural net reduces to
the equation for logistic regression:

\[ \hat{y} =\sigma(W^{(1)} X + b^{(1)}) \]

If a zero-hidden-layer neural net is also trained with log-loss, both the
model's representation and fitted parameters will be exactly the same as
logistic regression.

With one hidden layer ($L=2$) this expands slightly:
...
$\hat{\Theta} = \text{argmin} \, J(\Theta;X)$. One condition which must be true
at a local minima is that $\nabla_\Theta J = 0$. That gives us the equations:
...
derivatives with respect to a matrix (for W) or a vector (for b.) The [matrix
cookbook][MC] may help you understand this notation. 

Given the forward pass equations given above, we can easily calculate the 
partial derivatives for the individual components. You can verify for
yourself that:

\[ 
	\begin{split}
		\color{blue}
		\frac{\partial J}{\partial a^{(L)}} & \color{blue} = \frac{\partial J}{\partial \hat{y}} = y - \hat{y} \\
		\color{green}
		\frac{\partial a^{(i)}}{\partial z^{(i)}} & \color{green} = a^{(i)} \circ (1-a^{(i)}) \\
		\color{green}
		\frac{\partial z^{(i)}}{\partial a^{(i-1)}} & \color{green} = W^{(i)} \\
		\color{red}
		\frac{\partial z^{(i)}}{\partial W^{(i)}} & \color{red} = a^{(i-1)} 
	\end{split}
\]

Which are all straight-forward, except perhaps the derivative of the sigmoid
were we rely on the slightly non-obvious fact that $\sigma'(x) = \sigma(x)
(1-\sigma(x))$ which we proved in [Part 2][MLFS2].


To take a partial derivative of $J$ with respect to any parameter in any layer we can use
the chain rule. For $W^{(L)}$ we have:
...
\[ 
	...
	\color{green}
	\frac{\partial a^{(L)}}{\partial z^{(L)}} \color{black} \circ
	\color{red}
...
\[ 
	...
	\frac{\partial a^{(L-1)}}{\partial z^{(L-1)}} \color{black} \circ
	\color{red}
...
\[ 
	...
	\frac{\partial a^{(L)}}{\partial z^{(L)}} \circ
	\frac{\partial z^{(L)}}{\partial a^{(L-1)}} \circ
	\frac{\partial a^{(L-1)}}{\partial z^{(L-1)}} \circ
	\frac{\partial z^{(L-1)}}{\partial a^{(L-2)}} \circ
	\frac{\partial a^{(L-2)}}{\partial z^{(L-2)}} \color{black} \circ
	\color{red}
	\frac{\partial z^{(L-2)}}{\partial W^{(L-2)}}
\]

I hope you are starting to see the pattern here. As we go back layer by
layer, the left-most part of the equation (blue and green) for layer $i$ is always
the same as the blue and green part from layer $i+1$ plus two more new green factors.
...
at the "head" of the equation. At the (red) "tail", we always take a derivative
with respect to the parameters of interest. And in between we between we have a
growing (green) "body" of partial derivatives.
...
the "body" get to layer $i-$.  Note that in order to calculate $\color{green}
\partial a / \partial z$ we need to know the activations which are most easily
obtained by first performing a forward pass and remembering the activations for
...
[Cookbook][MC]). I think this a neat trick and one of the things that really
helped me understand the algorithm.
...
Exactly how we initialize the network turns out to be very important. If we
...
set; that would be fine if I wanted to talk about diagnosing and solving regularization
...
aren't a free lunch. But they do provide a framework for creating models
with low bias even on very large and problems... and then it's our job to
keep the variance in check.
...
prominently feature automatic differentiation as a core capability. The chain
