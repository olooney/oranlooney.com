...
gradient descent to deep neural networks: the [backpropagation algorithm][BA]. The
history of this algorithm appears to be [somewhat complex][BPH], as you can
hear from [Yann LeCun himself in this 2018 interview][YLC], but luckily for
us the algorithm in its modern form is not difficult - although it does require
a solid handle on linear algebra and calculus.
...
implement that backpropagation algorithm from scratch, implement a working
version, and to discuss how we can mitigate some of the dangers introduced when
we move beyond logistic to more complex representations.

Representation
--------------

A typical fully-connected feed-forward $L$-layer neural net is given
recursively in just four simple equations, the first and last of which merely
define how we get data in and predictions out of the network:
...
[matrix cookbook][MC] may help you understand this notation. 
...
\[ 
	\begin{split}
		...
		\color{maroon}
		\frac{\partial z^{(i)}}{\partial W^{(i)}} & \color{maroon} = a^{(i-1)} 
	\end{split}
\]
...
(1-\sigma(x))$ which we proved in [Part 2][MLFS2]. From now on, we will be
describing the equations for $\nabla J$ in terms of partial derivatives - 
if you want to know how to actually calculate anything concretely, refer to these
four equations.
...
\[ 
	...
	\color{black} \Bigg) \circ \Bigg( \color{green}
	\frac{\partial z^{(L)}}{\partial a^{(L-1)}} \circ
	\frac{\partial a^{(L-1)}}{\partial z^{(L-1)}} 
	\color{black} \Bigg) \circ
	\color{maroon}
