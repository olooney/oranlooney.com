...
have historical or pedagogical value: the [one-unit perceptron][PA], [linear
discriminant analysis][LDA], the [winnow algorithm][WA]. We will omit those and jump
straight to the simplest classification that is in widespread use: [logistic
regression.][LR]

[WA]: https://en.wikipedia.org/wiki/Winnow_(algorithm)
[LDA]: https://en.wikipedia.org/wiki/Linear_discriminant_analysis
[PA]: https://en.wikipedia.org/wiki/Perceptron
...
by Hosmer et al.][ALR]. But if you simply want to fit data and make predictions then
logistic regression is indeed a very simple model: as we'll see, the heart of
the algorithm is only a few lines of code.

Despite it's simplicity, it's important for three reasons. First, it can be surprisingly
...
final model with only slightly lower [generalization error][GE] than logistic
regression.  Obvious, this isn't *always* true, otherwise there would be
...
[GE]: https://en.wikipedia.org/wiki/Generalization_error
...
The third and final reason is that it cannot be solved with linear algebra
...
because it means we can calculate $\sigma'(x)$ with only a single multiplication
assuming that we already know $\sigma(x)$.
...
putting $y$ and $1-y$ in the exponent is merely a compact way of writing the two mutually exclusive scenarios
...
concretely demonstrate the value of each added complication. The layers will be:
...
which is the maximum likelihood estimate for $\Theta$.
...
state-of-the-art optimizers are usually available off-the-rack.
