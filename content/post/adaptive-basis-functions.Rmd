...
Today, let me be vague. We're going to go through a series of examples which
culminate in a very suggestive graph, which sort of slyly implies a certain
conclusion, without actually proving anything.

It's going to very handwavy, meant only to supply some basic intuition. But
what we sacrifice in rigour, we're going to make up for in generality; insofar
as we understand *why* modern machine learning (and here I mean deep neural
networks and boosting algorithms such as [XGBoost][XGB]) work so well, and
insofar as there is a "why" question that can be answered at all, this seems to
be it:

> For any given problem, there exists learned featured representations -- also
> known as adaptive basis functions -- which are better than any
> fixed/human-engineered set of features, even once the cost of the added
> parameters necessary to also learn the new features into account. 
...
The learned $k \times k$ kernels of a 2D CNN works better than Haar features or
Sobel filters. A one dimensional CNN and a spectrogram works better than MFCC.
An ensemble of weak learners works better than a single deep decision tree. And
so on.

Even as mythology, there are already plenty of cracks in the story. The biggest
...
uses Haar features, and is still widely used because it is so much faster than
CNN-based approaches.  While cutting edge CNN-based object detectors are
starting to achieve *acceptable* performance, they'll probably never be as fast
as Haar features, simply because they'll never be able to take advantage of the
integral image trick. 

But let's zoom out at look at the big picture instead of shopping for
counterexamples and limitations. At a high level, the story is clear. Again and
again, on wildly different kinds problem and for wildly different algorithms,
...
and mathematical analysis, and just throwing them out and replacing them with a
representation that the algorithm fits to the available data is a winning
strategy. It not only works, it often smashes straight through the glass
ceiling of performance where the previous generation of techniques had
...
hits a certain threshold of complexity? I think we *can* see the phenomenon for
ourselves in a context which is not too complex: learning to approximate a
...
Suppose we have a finite set of $N$ basis functions $\psi_1, ..., \psi_N$ and
let $V = \text{span} \{ \psi_1, ..., \psi_N \}$.  Then, for any function $g \in
V$, we can always write $g$ as a linear combination of basis functions:
...
For the examples below, we're going to need some target function. It should be continuous,
bounded on some closed interval, and it's also convenient if it's approximately zero on
both edges. The unit interval $[0, 1]$ is as good a choice as any. To make the function
...
To get warmed up, let's use this framework to calculate the best possible step
...
Again, these are only distinguished by their offset.
...
That's expected: Fourier series are pretty bad at approximating polynomials, and
...
function given an unlimited number of basis functions, this can require an
unreasonably large number of basis functions (and therefore parameters) before
...
well with only a handful of parameters.


Adaptive Basis Functions
------------------------

Finally, we come to our ringer.
...
\[ g(x) = \sum_{i=j}^N \beta_j \psi_j(x;\theta_j) \]
...
sigmoid is basically linear! It's like we're getting a two-for-one deal.
...
Certainly 7 fixed sigmoid functions would have done a very poor job. But
...
So here's the punchline: this representation -- a linear combination of
adaptive sigmoid functions -- is exactly the same as a [simple neural network][MLP]
with one hidden layer and a linear function for the output activation. Go back
and compare it to the model described in the [previous article on neural networks][MLFS3]
if you don't believe me!

Moreover, there is a [universal approximation theorem][UAT] which states that
this type of neural network can approximate *any* continuous function on
$\mathbb{R}^n$.

This isn't just some ivory tower theorem: it works quite well in practice too,
as we've just seen.  Not only can models on this form approximate an continuous
...
I should point out that we didn't use backpropagation to solve this network,
but for such a small model it hardly matters. In some sense, what we've done
here is party like it's 1970 and train a neural network using older methods.

There's another problem because the basis functions are in some sense
...
local minima which are not as good. 


Conclusion
----------

Did I convince you that adaptive basis functions quote-unquote "work?" I'm not
even sure I convinced myself. I'm left with the feeling that if I had just
...
is the trap of human-engineered features - while of course you *could* spend
...
They allow us to have very flexible representations that
can approximate essentially arbitrary functions with relatively few parameters.
There's no question these models can do more with less compared to traditional
models but it didn't come for free. We gave up convexity, we gave up linearity,
we gave up the ability to impose assumptions.  And in giving up these things,
we've also (temporarily, I hope) given up some of our ability to reason about
our models.

This, then, is the blessing and the curse of modern machine learning: adaptive
basis functions work. A blessing because we can reap the benefits of using 
them, and a curse because our lack of understanding hampers future progress.
...
as well understood as linear regression is today.
