...
> For any given problem, there exists a learned featured representation
> which is better than any human-engineered set of features, even once the added
> parameters necessary to also learn the new features is taken into into account. 
...
hits a certain threshold of complexity? It think it *is* possible to find
such elementary problems that demonstrate the power of learned representations.

A fairly simple example, which happily lends itself to easy visualization,
...
Given a random sample ${(Y_i, X_i)\}_{i=1}^n$ from the joint probability
distribution $D$, we wish to find a real-valued function $f : \mathbb{R} \mapsto \mathbb{R} $ such that:
...
\[ \hat{f} = \underset{f \in \mathcal{F} }{{\text{argmin}}} \sum_i^n (f(x) - E[Y|X])^2 \]

The question then becomes how we choose the family of functions $\mathcal{F}$ 
so that this optimization problem becomes tractable. The obvious answer is to
...
One easy way to create such a parameterization is to assume that f is
the weighted sum of $k$ fixed [basis functions.][BF].
Suppose we have a finite set of $k$ basis functions $\psi_1, ..., \psi_N$ and
let $V = \text{span} \{ \psi_1, ..., \psi_N \}$.  Then, for any function $g \in V$,
we can always write $g$ as a linear combination of basis functions:

\[ g(x) = \sum_{i=j}^N \beta_j \psi_j(x) \]

[BF]: https://en.wikipedia.org/wiki/Basis_function

Substituting into (1) above, we have an explicit loss functiOn:

\[ J(\mathbf{\beta}) = \sum_{i=1}^n (E[Y|X] - \sum_{j=1}{k} \beta_j \psi_j(x) )^2 \]
...
We will call the choice of parameters that minimize loss $\hat{beta}$

\[ 
    \begin{align}
        \hat{\vec{\beta}} & = \text{argmin}_\beta J(\mathbf{\vec{\beta}}) \\
        \hat{f}(x)  & = \sum_{j=1}^k \hat{\beta}_j \psi_j(x)
    \end{align}
\]
...
they are models where the basis functions themselves are parameterized by
some of the parameters of the model, and not just the weights out front.
...
here is pretend like it's 1970 and train a neural network using obsolete methods.
However, backprop can be viewed purely as a trick to make training faster - we
don't need it to understand the expressive power of this model. 
