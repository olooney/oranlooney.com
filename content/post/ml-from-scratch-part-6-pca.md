...
In the [previous article][MLFS5] in [this series][MLFS] we distinguished
...
In dimensionality reduction we seek a function $f : \mathbb{R}^n \mapsto
...
methods are more mature, if more limited. 

Linearity does not suffice to fully specify the problem, however. [Factor
Analysis][FA] also seeks a linear map, but takes a more statistical approach
and reaches a slightly different solution in practice. [Non-negative matrix
factorization][NMF] seeks a linear map represented by a matrix with no negative
elements - a restriction which PCA does not have.

I mention these other techniques to make the point that merely specifying that
$f$ should be a linear map underspecifies the problem, and we need to be
careful about what additional requirement we add if we're going to end up with
PCA instead of some other method.

Surprisingly, there are actually at least three different ways of fully
...
difficult to solve. But I digress.

Under any of these three definitions, the linear map $f$ that we seek will turn
...
*angles*.  These are ideal properties for a transform to have because merely
rotating an object or holding it up to a mirror never *distorts* it, but
simply gives us a different perspective on it.  

Imagine, for illustration's sake that you held in your hand an unfamiliar
...
insight into the nature of the original form.

<img src="/post/ml-from-scratch-part-6-pca_files/shape.png">

Orthogonal matrices are the safest and least distorting transformation that we
...
data without misrepresenting it.
...
Orthogonal matrices are always square, so $Q$ is an $m \times m$ matrix.
But multiplying by a square matrix doesn't reduce the dimensionality at all!
Why is this considered a dimensionality *reduction* technique? Well, it turns
out that once we've rotated our data so that it's as *wide* as possible along
the first basis vector, that also means that it ends up as *thin* as possible
...
the information (variance) of the data.  Of course, deciding *exactly* how many
dimensions to drop/retain is a bit tricky, but we'll come to that later.

For now, let's explore the mathematics and show how PCA gives rise to a
...
which we will write as the matrix product $\mathbf{q}^T \mathbf{x}$. This new
...
### An Ode to Lagrange Multipliers

Happily, a very general and powerful trick exists for translating constrained
optimization problems into unconstrained ones: [the method of Lagrange
multipliers][LM]. In 1780, [Lagrange][JLL] was studying the motion of
constrained physical systems.  At the time, such problems were usually solved
by finding a suitable set of [generalized coordinates][GC] but this required a
great deal of human ingenuity. 

As a concrete illustration, consider a bead on a stiff, bent wire.  

<img src="/post/ml-from-scratch-part-6-pca_files/bead_on_wire.png">

The bead moves in all three dimensions and therefore requires three coordinates
$x$, $y$, and $z$ to describe its position. However, it is also constrained to
the one dimensional path imposed by the shape of the wire so in theory its
position could be described by a single parameter $t$ describing its position
...
Lagrange developed a method by which this could always be done in an
essentially mechanical way, requiring no human insight. As just one example,
the technique is used in modern [physics engines][PE] - as new contact points
...
worry about finding an appropriate set of generalized coordinates. 

Lagrange's genius was to imagine the system could "slip" just ever so
...
solution to (6). Such is the magic of Lagrange multipliers.

But if (8) is just our constraint in a fancy new dress, how have we progressed
at all?  Because (7) is now unconstrained (and therefore more tractable!)


### Finding the Direction of Maximal Variation

Suppose the matrix $\mathbf{X}$ is an $N \times n$ matrix with $N$ rows where
...
Thus, to find the axes of maximal variation, it suffices to find the eigendecomposition
of $\mathbf{C}$.

Define $\mathbf{Q}$ to be the $n \times n$ *right* eigenvalue matrix (meaning
...
original form, I see that the covariance matrix is **not** diagonal.  That
...
features of $X'$ are independent.

This argument is much more brutish and perhaps too on the nose: we wanted a
...
sometimes calls "street statistics."


### A Brief Aside About Loadings 

At this point it may also be worth mentioning that multiplying by the left
eigenvector matrix $\mathbf{Q}$ is only one of two common ways to define the
...
of each eigenvalue. Using $\mathbf{L}^{-1}$ instead of $\mathbf{Q}$ to
transform from $\mathbf{X}$ to $\mathbf{X}'$ means the empirical covariance
...
projections from $n$ to $m$ dimensions, taking the first $k$ components of the
PCA transformation of $X$ minimizes the *reconstruction error*. Reconstruction
...
quirk of the Frobenius norm.

I won't go through this derivation here - you can find [a presentation
elsewhere][L25] if you're interested in the details - but it's an extremely
...
of components. This is commonly used with very large sparse matrices.
...
earlier [article on linear regression][MLFS1]. It's just as fast or faster than
...
discard these anyway, this is not such a bad deal!

Recall from that [previous article][MLFS1] our implementations for Householder
reflections and QR decompositions:
...
idealized form. There are really two pieces of book-keeping to implement:

1. We need to ensure than the data are centered 
2. Optionally "whiten" the data so that each feature has unit variance
3. put eigenvalues in descending order

Aside from these considerations, the entire `fit()` function is little more
than the handful of lines necessary to diagonalize the empirical covariance
matrix. All of the hard work is done inside `eigen_decomposition()`.
...
These data have two characteristics that we should consider carefully.

First, we can see that the features of this dataset are not on the same scale;
...
purposes of this visualization, we will manually whiten the data.
...
Second, it is clear from this plot that this dataset exhibits significant
[multicollinearity][MC]. Every feature exhibiting high correlations with
...
From the eigenvalues, we can see that the first few components explain most of
the variance:
...
The raw eigenvectors are hard to interpret directly, but if you like you can
...
component, among others, while "ash" contributes almost nothing to it.
...
Just as a cross check, we can plot the covariance matrix of the transformed
data the same way we did the raw data above:
...
And we can see the expected structure: eigenvalues descending from 4.7 to 0.1
...
This may seem a little silly, but in fact boiling it down to only a single
...
exists along a spectrum from poor to good quality? 

However, in this case, it seems like the first principle component does *not*
...
Now we can see that each class corresponds to a well-localized cluster with
...
single *curved* path that would connect these three regions in order or
ascending quality; this suggests that a non-linear dimensionality reduction
technique, say some kind of manifold learning like [t-SNE][TSNE], *might* be
able to reduce quality to a single dimension. But is that the representation it
would discover on its own, when trained in an unsupervised manner?
...
to visualize with standard plotting tools, but we can use pairwise 2D plots:
...
At this point in a real analysis, we would spend some time understanding what
...
quality. But there is an elephant in the room - the informal or seemingly *ad
hoc* method I used for deciding to use two components instead of one or three.
...
plot][SP] after a resemblance to the "elbow" that appears near the base of some
...
article][MLFS5]) [from scratch][MLFS], we should have a pretty good handle on
...
[PE]: https://gamedev.stackexchange.com/questions/117284/when-are-lagrangian-mechanics-useful-in-a-game
