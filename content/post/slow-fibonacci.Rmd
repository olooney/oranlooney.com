...
spectacularly inefficient implementation in.
...
In fact, the problem with the lambda calculus is that it is *too* simple: it is
difficult at first to see how anyone could *do* anything with it. Luckily for
us, there exists a set of macros which turn the lambda calculus into a much
higher level language. They were developed by [Alonzo Church][AC], the
inventor/discoverer of the lambda calculus, in parallel with the lambda
calculus itself. These macros provide a simple and concrete way of encoding
numbers, mathematical operators, boolean logic, and even data structures like
lists, maps, and trees. Today, this technique is called [Church encoding][CE].
If lambda calculus is machine code, then the [Church encoding][CE] is more like
C or LISP.
...
Before any of that, though, we need an implementation of the lambda calculus
itself. For the purposes of this article, we're going to take a non-standard
...
in this article will run in any Python 3 interpreter so readers can follow
along and try their own experiments if they like. I shall call this the
...
In the next section I will describe this subset more formally and describe how
it maps to the lambda calculus. In this section, I'll do a quick high-level
overview and give an example of where we are going.

Python already has the two main operations we need anonymous lambda functions
(`lambda x: x`) and function application (`f(x)`) so this is perfectly natural.

In addition, we will implement features by introducing *definitions*, which are
basically macros with a human readable name that expand out to lambda calculus
expressions. Here is an example of a definition:

    plus = lambda m: lambda n: lambda f: lambda x: m(f)(n(f)(x))

Definitions get substituted into other expressions very much like running "Find
and Replace All" in an IDE or 

    s/plus/(lambda m: lambda n: lambda f: lambda x: m(f)(n(f)(x)))/g

in VI or sed.  By introducing definitions, we will
incrementally build up a surprisingly high-level and expressive language.

This is good time to exhibit what the Fibonacci function will look like in the
Pythonic lambda calculus when we're done.  
...
As always in Python, `#` indicates an non-executable comment. It shouldn't be
to hard to see the basic shape of the familiar, friendly Fibonacci function in
there. If you have any LISP, it will even look vaguely familiar, although the
parentheses are in slightly different places. (In LISP, function application
looks like `(minus n 2)` instead of `minus(n)(two)`.)

In the next section, we will briefly describe the lambda calculus and the
Pythonic variant we will be using.
...
function) that has or ever will be written.  (Since the lambda calculus is
[Turing complete][TC], this can be interpreted as implying that all computation
can be reduced to abstraction and application; all else is vanity and grasping
at wind.) 
...
in and hit ENTER to see the answer. In most cases values `3` and `4` are too
...
TODO: this is the job of the computer (human, machine or otherwise)
...
need for the $\alpha$-replacement rule.  The $\beta$-reduction rule captures the
...
of the lambda calculus is to prove that these two operations suffice to define
...
definition.  This takes only a finite number of steps and is guaranteed to
...
Incidentally, by exhibiting a model of the lambda calculus in Python, we've
shown that Python is Turing complete, although to complete the proof we would
need to show that Python's execution of these lambda expressions is equivalent
to the $\beta$-reduction rule. However this would take us far afield of our
main purpose, which is to exhibit a practical, runnable example of a
non-trivial program written entirely in the lambda calculus.
...
doesn't prevent us from thinking of some functions as values when it is
convenient for us. Consider the objects of object-oriented programming - an
...
high-level and informal for now. (I'll have more to say about types when we
discuss recursion.)

Since we are keeping things informal, we can use an intuitive definition: the
type of a lambda calculus expression is roughly the number and types of the
parameter it would normally be called with. In high level languages this is
often called the function signature. The two Church Booleans we defined both
have the same type - they expect to be called with two arguments and will then
return one or the other of those two arguments unchanged.

Consider the following function, which takes a Boolean argument:

    lambda p: p(ifvalue)(elsevalue)

It will "work" equally well if `p` is `true` or `false` - only the behavior
will be different. If `p` is `true`, it will return the `ifvalue` and if `p` if
`false` it will return the `elsevalue`...  in other words, it has the exact
same semantics as a typical if/else statement!
...
which we'll see much more of, is simple: use Church encoding to somehow
translate your input values into lambda expressions. Pass those into a lambda
expression which represent your program; then reverse the Church encoding
...
number $n$ to be a binary (taking two arguments) function which takes a
function `f` and an arbitrary value `x` and applies `f` to `x` exactly $n$
times:
...
a Python [Callable][PC]. All I see on my screen when I run the above snippets is
...
symbol table. We can translate this half-way into Pythonic lambda calculus
simply by refactoring every named function into a lambda function:
...
is that our Church Boolean pseudo-`if-else` statement is not quite the same as
`if-else` other languages. If we use `if-else` in a language like C or Python,
the code inside the selected branch will be executed, but code in the other
branch will *not even be run.* However, in the Pythonic lambda calculus, if we
write:
...
completely regardless of the value of `some-predicate`. This is called "eager"
evaluation and Python *always* eagerly evaluates all arguments of a function
call. Therefore in Python, we will always compute *both* branches, and only at
the very end will we discard one of the values. Normally, this wouldn't cause
serious problems because the answer would always be the same (it would just be
a little slower) but it becomes a serious problem in the case of recursion
because the `else` branch is *always* evaluated, which calls the function
again, which calls the function again, and so for forever.

One response would be to give up on Python and go abuse some other language
which has lazy evaluation TODO. Alternatively, we could write an interpreter
for the lambda calculus which implements $\beta$-reduction rule, side-stepping
Python entirely.
...
We now have all the necessary tools. All that remains is to implement
...
We can test that it is correct by exhibiting the Fibonacci numbers up through $F_20$:
...
indefinitely, with each layer costing us another factor of a thousand. It
would be reminiscent of this art project where a long gear chain is used
to build a machine which will take 13.7 billion years for the final gear
to complete one rotation. The machine has a electric motor happily whirring 
away at one end, and a solid block of concrete at the other; but because
gear steps the revolution speed down by a factor of 20, the motion is infinitesimal
...
Perhaps you don't believe that this is really a lambda calculus program; after all, it has
all those "definitions" which look suspiciously like named functions and in fact are being
treated as named functions by the Python interpreter! Very suspicious. 

I can show this is not the case by doing a search-and-replace for each name and replacing
it with its body, and repeating that until nothing is left. Below is the step-by-step working
out, with one version per line; the last line contains no definitions (expect our `PIR` hack)
and instead is composed entirely of `lambda` functions, function calls, and bound variable names.
...
Here is a visualization of the full Fibonacci program (click for a larger image.)
This shows every node (either a variable, function call, or lambda abstraction) as
a binary tree.
...
[`cons`][CONS] is taken as primitive. Of course, the [pair data structure][LCP]
can easily be defined in the lambda calculus as well.
...
boils down to the [impedance mismatch][IM] between the lambda calculus and the
kinds of electronic computers we can actually build. We can't actually just
...
reasons that aren't entirely clear, the lambda calculus has been a fruitful
model for thinking about high-level, expressive ways of computing, while the
Turing machine has been a fruitful model for how to design and actual computing
machines. While in theory we can always switch from one model of computation to
another, in practice this can involve a three or four order of magnitude
...
useful high level abstractions. This turns out to be much harder than simply
getting something that works at all in a performance naive way, which is
basically what we've been doing in this article.
