...
expressions. Here is an example of a definition, the contents of which we will
return to later:
...
By introducing definitions, we will incrementally build up a surprisingly
...
With those preliminaries out of the way, here is our goal: the Fibonacci
function implemented entirely in the Pythonic lambda calculus:
...
As always in Python, `#` indicates an non-executable comment. As we'll soon
see, lambda calculus doesn't have explicit an `if/else/then` branch statements,
so I've used comments to make the flow of control a little clearer.
...
In the next section, we will describe in more detail the lambda calculus and
the Pythonic variant we will be using.
...
really our only way of defining data structures and passing state around.
...
constants; lowercase `true` and `false` appear to Python as callable lambda
objects, not objects of type `bool`.
...
lambda calculus][TLC] but we don't need it to actually compute stuff.)
...
too. You can try manually working out the $\beta$-reduction of `pred(two)`
...
languages like [Haskell][HKL] support lazy evaluation!) Alternatively, we could
write an interpreter for the lambda calculus which implements $\beta$-reduction
rule, side-stepping Python entirely.

These are good options, and for someone writing a very complete implementation
of the lambda calculus they would be necessities. But today we're only concerned
to write one function, the Fibonacci function, so we can use a much simpler
hack to prevent infinite recursion, which I will abbreviate `PIR`:
