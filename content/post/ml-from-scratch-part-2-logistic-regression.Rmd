...
because it means we can calculate the derivative with a single multiplication.
...
Let's say $Y$ is a discrete random variable with support ${0, 1}$ and $X$ is a
random vector.  Let's assume that the joint probability distribution $F_{X,Y}$
of $X$ and $Y$ has the following form for some unknown real-valued vector
$\Theta$.
...
We're using a simple notational trick to write this compactly: because $y \in
...
that's merely a compact way of writing the two scenarios without an explicit
if/else.

We can simplify this expression by taking the log of both sides and working
with the *log-likelihood* from now on. 
...
Because $ln$ is monotonic, it suffices to minimize $\ell(\Theta; X, y)$ with
respect to $\Theta$ in order to find the maximum likelihood estimate.  Because
$\ell$ convex and has a continuous derivative, we can find this maximum by
solving $\nabla \ell = 0$.
...
This doesn't admit a closed form solution, but it is amenable to numerical
methods. 

For our implementation below it will be convenient to have this in vectorized
form. Leaving behind the statistical context and no longer using random variables,
let $X$ be a $N \times m$ real-valued matrix (not a random variable) such that
each row of $X$ is $X_i$ from above, and let $y$ the corresponding non-random
real-valued $N$-vector, the above equation becomes:

\[ \nabla_{\Theta} \ell(\Theta; X, y) = (X \circ  (y - \hat{y}))^T \textbf{1} \]

(Here, we use the notation that $\textbf{1} = (1, 1, ..., 1)$ is the vector
consisting of all ones and $\circ$ is the Hadamard product.)
...
believe the true population distribution *really* behaves as if it were using a
logistic link function, or is this an arbitrary choice we made solely for
mathematical convenience? As far as I can tell, it's the later case. More
rigorous attempts to justify the choice of link function end up making a
different choice, such as probit regression which uses the CDF of the normal
distribution. However, in practice these curves are extremely similar, and if
you showed me a plot of each side-by-side I could not for the life of me tell
you which is which.
...
closed form, the obvious numerical method is gradient descent, also called
batch gradient descent.  This is an iterative method that simply updates an
approximation of $\hat{\Theta}$ by taking a smalls step in the direction of
steepest descent.

\[ \hat{\Theta}_{i+1} := \hat{\Theta}_i - \alpha \nabla \ell |_{\hat{\Theta}_i} \]
