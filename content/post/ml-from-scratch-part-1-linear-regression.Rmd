...
algorithms. For example, the output layer of a deep neural network trained for regression
...
same as maximizing $L$. Any constant $C$ has no effect whatsoever
...
analogous to finding the critical points of a univariate function by looking
...
either *rotate* by the angle between the vectors \(\cos^{-1} \frac{a_1 \dot
e_1}{||a_1||} \), or we can *reflect* across the line which bisects the two
