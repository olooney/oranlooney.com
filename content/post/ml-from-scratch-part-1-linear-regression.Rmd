...
model we started with. This isn't general true, but it is *exactly* true for linear regression
...
shaped like a big, round bowl.  Because of these
...
\[ 
\nabla_\Theta J = \nabla_\Theta \; 
\mathbf{y}^T \mathbf{y} - (\mathbf{X}\Theta)^T \mathbf{y} - \mathbf{y}^T \mathbf{X}\Theta + \Theta^T (\mathbf{X}^T \mathbf{X}) \Theta \]
