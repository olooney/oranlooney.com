...
random variables they are the  given data used to fit the model. We can then
ask, what is the likelihood of obtaining the data $(X, y)$ from an arbitrary
parameter vector $\Theta$? By rearranging our equation as $y_p - X_p\Theta =
...
What we did above was essentially a short sketch of parts of the [Gauss-Markov
theorem][GMT] the OLS solution *is* the MLE for the particular statistical
model we started with. The full Gauss-Markov theorem proves a bunch of other
nice properties: it turns out this estimator is unbiased and we can even say
...
for points where the derivative is zero. It's not at first obvious how to
...
Why is $Rx = Q^T b$ easy to solve? Well, the right had side is just a vector. R
...
So hopefully you're convinced by now the $QR$ form is desirable. But how do we
...
side of the equation by $(R^T)^{-1}$ and cancel the $R^T$ on both sides, leaving only:
