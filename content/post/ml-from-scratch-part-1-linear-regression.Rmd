...
deep theoretical ideas about machine learning with an example that is not
...
over because it involves more linear algebra than can be generally assumed, 
and because the clever exact solution we derive doesn't generalize well to
other machine learning algorithms or even to mild variants of linear regression
...
general optimization algorithms (i.e., [SGD][SGD]) that work for a wide variety
of models and scale reasonably well to vast amounts of data. This allows
researchers to iterate rapidly on the structure of the model without needing to
spend a lot coming up with a clever algorithm which solves their special case
efficiently.  It's good software engineering: it avoids [premature
optimization][WTO] and promotes good [separation of concerns][SOC].  Still,
history has shown that for any given optimization problem, there probably is a
specialized algorithm that leverages the specifics of the problem to achieve an
order of magnitude better performance. For example, John Platt's [Sequential
Minimal Optimization][SMO] beat earlier, more general algorithms by such a wide
margin that for a decade (1998-2009?) SVMs were one of the most promising
approaches to machine learning. Today (2019) the machine learning industry is
kind of in rapid prototyping mode, leveraging the flexibility and composability
of deep neural networks to experiment with endless numbers of novel
architectures.  However, as our understanding of which architectures work best
for particular problems matures, the industry will likely tip back in favor of
researching specialized algorithms. If we are interested in understanding
machine learning from scratch we should be prepared to study specialized
algorithms when and where they arise naturally.
...
posed this problem, we will afterwards ignore the statistical altogether and
focus on numerical methods for solving the optimization problem. This is often
how statistics is most useful in machine learning - a good way to first
approach very abstract problems, but less necessary once the problem has been
made concrete.
...
Here, note that $v v^T$ is the *outer* product of $v$ with itself so is a square matrix with elements $v_i v_j$. For example,
if $v = (\frac{1}{\sqrt{2}} \frac{1}{\sqrt{2}} 0)^T$ (the 45&deg; line in the xy-plane) we get:
