...
\[
	...
	\Delta I_G & = I_{parent} - p_{left} I_{left} - p_{right} I_{right}
	\end{align}
\]
...
There's a competing metric called information gain which is often used instead
of Gini impurity. There's a long derivation where entropy is defined as the log
of the probability mass function of a multinomial distribution, information is
defined as the opposite of entropy, and information gain is defined as the
difference between the entropy of the parent node and the weighted sum of the
entropies of all child nodes, but long story short, for the simple binary case
the formula looks like this:

\[
	\begin{align}
		       H & = -p \ln p - (1-p) \ln (1-p) \\
		\Delta H & = H_{parent} - p_{left} H_{left} - p_{right} H_{right}
	\end{align}
\]

Modulo a scaling factor, entropy has almost the same shape as the Gini impurity:

```{r gini_info, echo=FALSE}
curve( 4*x*(1-x), xlim=c(0, 1), ylim=c(0, 1), col='blue', xlab="p", ylab="Normalized Score")
curve( (1/C) * 0.5 * (-x*log(x) - (1-x)*log(1-x)),n=1000, add=TRUE, col='darkgreen')
...
Decision trees trained on one or the other will often be identical and
usually have identical performance, so we will use Gini impurity because
it is slightly cheaper to calculate a square than a log.
