...
answer is to use a [model of computation][MOC] which is not *deliberately*
designed to be slow but which in practice is quite slow, usually because the
designer had something quite different than performance in mind.

While there are [several][TM] to [choose][MRF] from, I selected the
[$\lambda$-calculus][LC] (read "lambda calculus") as being particularly easy to
write an spectacularly inefficient implementation in.
...
language. While [Alonzo Church][AC] was inventing the $\lambda$-calculus itself
...
called [Church encoding][CE].  If $\lambda$-calculus is machine code, then the
...
implementation of a function which finds the $n$-th Fibonacci number,
implemented in vanilla [Python][P]:
...
using the anonymous function syntax (e.g., `lambda x: ...`) and function calls
(e.g., `f(x)`).
...
then revert to proper $\lambda$-calculus at any time by simply expanding out
all the definitions like macros. 
...
lambda y: x * y` is ultimately defined in terms of the built-in multiplication
operation `*`, and `x` and `y` are are of type `int`. This won't do; indeed,
...
In plain english, we build up expressions recursively out of variables, lambda
...
Where the `<expression>` on the right is restrictions to have no free
...
an object is nothing *but* a collection of functions, but we usually think of
objects as values.  
...
if `false` it will return `b`. In other words, it has the same semantics
as an if/else statement or the [`?:` ternary operator][TO].
...
The first time `greet()` is called, it is called with the `1` we passed in. Each
...
[comes close][RT] to the idea that numbers should literally be their own `for`
loops. Even LISP and Haskell require recursion, a separate [map][HM] function,
or a [loop macro][LLM]. (For code readability alone this is probably a good
...
Following [Peano][PA], the first thing we need is a successor function which can
...
$\lambda$-calculus, we will rename `recursive` to `Y` - it is indeed the famous
...
`Y`, I have no idea - it's just the [standard symbol][LCS].
...
is of course a terrible algorithm; it's the same one I called `naive_fib()` in
my [earlier article][FFFF] on optimizing the Fibonacci function. However, this
...
This is one thousand times slower than the same naive algorithm in implemented
...
recently as 80 years ago this would have been state-of-the-art.
...
chained together, the motion is infinitesimal by the end.
...
If we had carried on defining signed numbers, pairs, lists, associative maps,
...
Church numerals fast, but we *can* make a machine which can perform
mathematical operations on two 64 bit numbers in a single cycle. We can't write
a performant $\beta$-reducer, but we *can* write a block of machine code which
calls functions by using a [calling convention][CC]: pops a certain number
arguments off the stack when called and returns a value on the stack or in a
particular register. And so on.

For reasons that aren't entirely clear to me, the $\lambda$-calculus has been a
...
Which isn't to say there aren't practical languages very much inspired by and
very close to the $\lambda$-calculus. LISP and Haskell come to mind. After
watching some of the [SICP lectures][SL] on Youtube, I'm convinced the only
reason Harold Abelson didn't just teach the whole course in pure
$\lambda$-calculus is because computers back then were still slow enough that
...
spirit to the $\lambda$-calculus (as opposed to something like C, which uses a
[pointer machine][PM] model which is only slightly higher level than a Turing
machine.) It's never been more important to expose CS students to the idea
and modes of thinking inspired by the $\lambda$-calculus and I think it's
importance will only grow as we can better afford the cost of abstractions.

As always, all the [source code][SC] for this article is free and open source. 
...
[FFFF]: https://www.oranlooney.com/post/fibonacci/
...
[HM]: https://zvon.org/other/haskell/Outputprelude/map_f.html
...
[LLM]: https://cl-cookbook.sourceforge.net/loop.html
...
[PM]: https://en.wikipedia.org/wiki/Pointer_machine
...
[SL]: https://www.youtube.com/watch?v=2Op3QLzMgSY
[SUT]: https://xunitpatterns.com/Test%20Spy.html
...
[TO]: https://en.wikipedia.org/wiki/%3F:
...
[SC]: /post/slow-fibonacci_files/slow_fibonacci.py
