...
$$ \mathbf{C} = \frac{\mathbf{X}^T \mathbf{X}}{n} $$

$$ f = \mathbf{q}^T \mathbf{C} \mathbf{q} + \lambda (1 - \mathbf{q}^T \mathbf{q}) $$

$$ \nabla f = 2 \mathbf{C} \mathbf{q} - 2 \lambda \mathbf{q} $$
...
Because the covariance matrix $\mathbf{C}$ is real-valued, symetric, and positive
...
be uncorrelated. Therefore I want the covariance matrix of my data to be diagonal.
However, when I estimate the covariance matrix for my data in their original form,
I see that the covariance matrix is **not** diagonal, which means my original
...
The simplest and most structure preserving way to do this is with a rotation, e.g., an orthogonal matrix. To
find such a transformation, I will diagonalize the covariance matrix $\mathbf{C}$. This can be done
by finding the eigen-decomposition of $\mathbf{C}$.

The resulting right eigenvector matrix $\mathbf{Q}$ can be applied to $X$. 

$$ \mathbf{X}' = \mathbf{X} \mathbf{Q} $$

TODO: prove that C' is diagonal.


Loadings vs eigenvectors
https://stats.stackexchange.com/a/92512/48250
https://stats.stackexchange.com/a/143949/48250
...
http://people.csail.mit.edu/dsontag/courses/ml12/slides/lecture25.pdf

This is an extremely powerful point of view which gets at the heart of the dimensional
reduction strategy. 

TODO: analogous to each component explaining a certain percentage of the variance.

I find it extremely interesting that PCA can be motivated in so many different
ways. These three are all completely valid viewpoints on what might define
a "good" dimensional reduction algorithm, and all three come to the same conclusion:
you need to find the eigenvectors of the covariance matrix.
