...
fact that the three models of computation proposed by Turing, Chuch, and Gödel
...
multiple intuitively attactive directions likewise suggests that it is a discovery.)
...
*angles*.  These are very nice properties for a transform to have! Merely rotating
an object doesn't really distort it, but simply gives us a different
perspective on it. In this sense, PCA is the least distorting transformation
that we could apply. 
...
by finding its eigen-decomposition. Therefore, I need to find the eigen-decomposition $\mathbf{C} = \mathbf{Q} \mathbf{\Lambda} \mathbf{Q}^T$.
...
let's turn our attention to the concrete problem of implementing eigen-decomposition from scratch.
