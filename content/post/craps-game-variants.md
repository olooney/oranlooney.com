...
Anyone who's ever calculated the chances of winning at [craps][C] may remember
being surprised by how fair the game is.

(**Spoiler Warning** If you *haven't* calculated these odds but feel you might
enjoy the challenge, you should do so before reading on, because I'm about to
spoil it for you rather thoroughly in the name of exploring a more general
case. A full solution may also be found in the rather excellent book, [Fifty
Challenging Problems in Probability with Solutions][50PP].)
...
any deep knowledge of mathematical.

Concretely, the exact odds are $\frac{244}{ 495} \approx 49.29\%$: That's less
than 0.71% away from perfectly fair -- and of course no reputable casino would
ever host a game that was *perfectly* fair.  (Note that this means that the
...
Honestly its so close to 50% it's actually a bit of a pain to test empirically:
...
adjusting his rules to try and find the perfect game. 
...
are called 7-11s, and not 6-10s? 
...
To answer this question we'll need a little bit of math and fair amount of
Python.
...
the same as outcome {1,4} because they both sum to 5.

A game of Craps has two phases: A "come out" phase for the first dice roll, and
an "on point" phase for all subsequent dice rolls in this game. In the "come
out" phase, if the player rolls a 7 or an 11, they immediately win. if the
player rolls a 2, a 3, or 12, they immediately lose. For any other roll, their
first dice roll becomes their "point" - the point number is fixed by that first
"come out" roll and remains the same until the end of the game. The player then
repeatedly rolls until one of two things happen: either they roll a number
which matches their "point" and they win, or they roll a 7 and they lose. 
...
Instead he "passes" and is now "on point." His next roll is an 11. Because we
are no longer in the come out phase, 11 has no special meaning. The only two
numbers that matter are his "point" (5) and to the "seven-out" (7). Since 11 is
neither of these, he rolls again and keeps rolling until eventually he rolls a
5, winning the game.

It's clear that the numbers play different roles. 7 and 11 are the "naturals"
that let you win immediately in the come out phase; 2, 3, and 12 are the
"craps" which let you lose immediately in the come out phase; and 7 is the
"out" that causes a loss in the "on point" phase. There is no particular
parameter for the "point" because this is decided by the "come out" roll.
...
true, it would be possible to enter the "come out" phase with your "point" the
...
Where we can recursively define the function $\text{onpoint}()$ as:
...
We can also represent the p.m.f. of $\mathcal{D}(6) + \mathcal{D}(6)$ using a
simple dict.
...
probability; in other words, all possible outcomes. 
...
a Monte Carlo simulation or an approximation: using `Fraction()` and summing
over the finite support gives us the exact answer.
...
With our class define, we can test it by feeding it the original parameters
...
If you haven't done so already, this is a good place to download the Jupyter
[notebook][CNB] and try some variants by hand to build intuition.

## Generating Variants

We next turn our attention to generating all variants. This is a two step
...
We're going to be generating a *lot* of partitions so it behooves us to have a
...
complex so we will have to leave that for some future article.
...
the rule "closest to 50% while still strictly less." In Python we usually use
the [decorate-sort-undecorate][DSU] idiom for such problems.
...
I'm fairly certain its a crime to conduct an analysis of a game of chance
without including a Monte Carlo simulation and since I do not want to be tried
by a court in Prague let's confirm our results with a simple simulation.

The simulation code itself is obvious and straight-forward:
...
constructed many variants that were *much* fairer. However, now I am even more
